...
interface PendingReferralData {
  currentBalance: number;
  transferReqDTO: any[];
  transactionDTO: TransactionDTO[];
}

const PendingReferralTab: React.FC = () => {
  const [referralData, setReferralData] = useState<PendingReferralData | null>(null);
  ...
  useEffect(() => {
    const fetchReferralData = async () => {
      try {
        ...
        console.log("Fetching pending referral data for userProfileId:", id);

        const response = await fetch(
          `/api/payment/getUnclearedReferalPoint/${id}`,
          {
            method: "GET",
            headers: { accept: "*/*" },
          }
        ...
        const data: PendingReferralData = await response.json();
        setReferralData(data);
      } catch (error) {
        console.error("Error fetching pending referral data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-1 md:p-6">
      ...
      <div className="bg-white shadow-md rounded-2xl p-6 border border-gray-100">
        {loading ? (
          <p className="text-gray-500 animate-pulse">Loading pending referral points...</p>
        ) : (
          <>
            <div className="flex justify-between items-center mb-6">
              <div>
                <p className="text-gray-500 text-sm">Pending Referral Points</p>
            ...
            <div>
              <p className="flex items-center gap-2 text-gray-700 font-semibold mb-4">
                <FaHistory className="text-gray-500" /> Pending Referrals
              </p>
              ...
              {referralData?.transactionDTO?.length ? (
                ...
                <p className="text-gray-500">No pending referrals yet.</p>
              )}
            </div>
          </>
        )}
...
export default PendingReferralTab;
